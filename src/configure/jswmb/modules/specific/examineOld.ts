    ...
    components:[
        {
            modelName:'content', type: 'AssemblyQuery',components:[
                {modelName:'query',type: 'AssemblyFrom',components:[
                        {modelName:'content',name:"type",label:"考察项目",type: 'ComponentsInput'},
        ...
        },
        {
            modelName:'content',type: 'AssemblyContent', components:[
                ...
                {modelName:'content',name:"photo",label:"问题照片",type: 'ComponentsInputImage'},
            ...
            tableBtn : [
                ...
                {label:"网格单位审核",show:true,click:function(data:any){
                        let edit = utils.getStore().openDialog({name:'examine'})
                        edit.query()
                    },jurisdictionJson: [
                        {row: [{process: '待网格单位审核'}],user: [{r_name: "网格单位-审核人员"}]}
                    ]
                },
                {label:"下发",show:true,click:function(data:any){
                        ...
                        {row: [{process: '网格单位审核成功'}],user: [{r_name: "创建办-审核人员"}]},
                        {row: [{process: '取消下发'}],user: [{r_name: "创建办-审核人员"}]},
                        {row: [{process: '下发申诉'}],user: [{r_name: "创建办-审核人员"}]}
                    ]
                },
                {label:"取消下发",show:true,click:function(data:any){
                        let edit = utils.getStore().openDialog({name:'delete3'})
                        edit.query()
                    },jurisdictionJson: [
                        {row: [{process: '下发'}],user: [{r_name: "创建办-审核人员"}]}
                    ]
                },
                {label:"下发申诉",show:true,click:function(data:any){
                        let edit = utils.getStore().openDialog({name:'edit3'})
                        edit.query()
                    },jurisdictionJson: [
                        {row: [{process: '下发'}],user: [{r_name: "责任部门-普通人员"}]}
                    ]
                },
                {label:"二次下发",show:true,click:function(data:any){
                        let edit = utils.getStore().openDialog({name:'edit4'})
                        edit.query()
                    },jurisdictionJson: [
                        {row: [{process: '下发'}],user: [{r_name: "责任部门-审核人员"}]}
                    ]
                },
                {label:"申请延期",show:true,click:function(data:any){
                        let edit = utils.getStore().openDialog({name:'edit5'})
                        edit.query()
                    },jurisdictionJson: [
                        {row: [{process: '下发'}],user: [{r_name: "责任部门-普通人员"}]},
                        {row: [{process: '二次下发'}],user: [{r_name: "责任部门-普通人员"}]},
                        {row: [{process: '延期'}],user: [{r_name: "责任部门-普通人员"}]}
                    ]
                },
                {label:"延期",show:true,click:function(data:any){
                        ...
                        {row: [{process: '申请延期'}],user: [{r_name: "创建办-审核人员"}]}
                    ]
                },
                {label:"申请销号",show:true,click:function(data:any){
                        let edit = utils.getStore().openDialog({name:'edit6'})
                        edit.query()
                    },jurisdictionJson: [
                        {row: [{process: '下发'}],user: [{r_name: "责任部门-普通人员"}]},
                        {row: [{process: '二次下发'}],user: [{r_name: "责任部门-普通人员"}]},
                        {row: [{process: '申请延期'}],user: [{r_name: "责任部门-普通人员"}]},
                        {row: [{process: '延期'}],user: [{r_name: "责任部门-普通人员"}]},
                        {row: [{process: '销号失败'}],user: [{r_name: "责任部门-普通人员"}]}
                    ]
                },
                {label:"审核销号",show:true,click:function(data:any){
                        let edit = utils.getStore().openDialog({name:'examine2'})
                        edit.query()
                    },jurisdictionJson: [
                        {row: [{process: '申请销号'}],user: [{r_name: "创建办-审核人员"}]},
                    ]
                },
        ...
        {modelName:'dialog',name:"info",dialog_width:"970px",type: 'DialogEdit',components:[
                ...
                {modelName:'from',type: 'AssemblyFrom',name:"base1",label:"整改记录",components:[
                        {modelName:'content',name:"list",label:"详情",type: 'ComponentsInputTable',style:{width: "930px"},label_show: false,value_default:[],components:[
        ...
        {modelName:'dialog',name:"edit3",apiName_save:"updateState6",dialog_width:"970px",type: 'DialogEdit',components:[
                {modelName:'from',type: 'AssemblyFrom',components:[
                        {modelName:'content',name:"appeal",label:"申诉原因",type: 'ComponentsInputSelect',ajax_url:"department",props_label:"name",props_value:"id"},
                        {modelName:'content',name: 'id', label: 'id',type:'ComponentsInputHidden'},
                    ]},
            ]},
        {modelName:'dialog',name:"edit4",apiName_save:"updateState7",dialog_width:"970px",type: 'DialogEdit',components:[
                {modelName:'from',type: 'AssemblyFrom',components:[
                        {modelName:'content',name:"department_id",label:"主要责任部门",type: 'ComponentsInputSelect',ajax_url:"department",props_label:"name",props_value:"id"},
                        {modelName:'content',name: 'id', label: 'id',type:'ComponentsInputHidden'},
                    ]},
        ...
        {modelName:'dialog',name:"edit5",apiName_save:"updateState8",dialog_width:"970px",type: 'DialogEdit',components:[
                {modelName:'from',type: 'AssemblyFrom',components:[
                        {modelName:'content',name:"delay_file",label:"延期资料附件",type: 'ComponentsInputUpload'},
                        {modelName:'content',name: 'id', label: 'id',type:'ComponentsInputHidden'},
                    ]},
            ]},
        {modelName:'dialog',name:"edit7",apiName_save:"updateState9",dialog_width:"970px",type: 'DialogEdit',components:[
                {modelName:'from',type: 'AssemblyFrom',components:[
