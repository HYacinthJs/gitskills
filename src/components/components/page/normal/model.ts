 ...
 * PageNormal 主页面
 */
export class PageNormal extends Page {
    ...
    componentDefault(data:{ [key: string]: any}){
        ...
        if(initMenu){
            this.addComponents(<Layout>utils.createComponents({ modelName:'menu', type: 'AssemblyMenu'}))
        }
        if(initHeader){
            this.addComponents(<Layout>utils.createComponents({ modelName:'header', type: 'AssemblyHeader'}))
        }
        if(initBreadcrumbs){
            this.addComponents(<Layout>utils.createComponents({ modelName:'breadcrumbs',type: 'AssemblyBreadcrumbs'}))
        }
        if(initFooter){
