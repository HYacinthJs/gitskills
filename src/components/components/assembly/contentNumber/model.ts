 ...
 * 主体内容
 */
export class AssemblyContentNumber extends Layout {
    constructor(data:{ [key: string]: any}) {                                               //构造函数
        ...
        if(utils.isUndefined(data.toolBtn)){
            this.toolBtn = [
                ...
                {label:"删除",jurisdictionJson: [{jurisdiction: [{name: "deleteAll"}]}],type:"danger",
                    click:function(data:any){utils.getStore().openDialog({name:'deleteAll',title:"删除"})}},
            ]
        }
    }
    //初始化表格要求的数据内容
    ...
    objectSpanMethod({ row, column, rowIndex, columnIndex }:any) {
        if(columnIndex === 0 ) {
            // this.tableData  修改
            const _row = (this.flitterData(this.value).one)[rowIndex]
     ...
     * 操作栏宽度
     */
    private _operation_width: string = "120";
    //操作栏宽度
