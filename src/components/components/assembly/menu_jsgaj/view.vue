        ...
        <el-scrollbar style="height:100%">
            ...
            <div class="logo" v-if="!current.isCollapse">
                <a href='javascript:;' style="margin:0px;height: 100%;width: 100%;" @click='public_goto_home()'>
                    <img :src="public_getSystemImg({name:'menu_logo'})" style="margin:0px;height: 100%;width: 100%;"
                        alt=""></a>
            </div>
            <div class="logo-icon text-center" v-else style="height:50px;">
                <a href='javascript:;' @click='public_goto_home()'><img :src="public_getSystemImg({name:'menu_logo2'})"
                        style="margin:0px;height: 100%;width: 66px;" alt=""></a>
            </div>
            <el-menu class="el-menu-vertical-demo" @select="goto" @open="open2" @close="close2"
                :collapse="current.isCollapse">
                <template v-for="menu in menuList" class="item2">
                    <el-menu-item :index="menu.menu_id+''" :key="menu.menu_id"
                        v-if="!menu.children&&menu.menu_linkurl.length>0">
                        <img :src="my_getSystemImg(menu)" class="menu_icon" />
                        <span slot="title" v-if="!current.isCollapse">{{$t(menu.menu_name)}}</span>
                    </el-menu-item>

                    <el-submenu :index="menu.menu_id+''" :key="menu.menu_id"
                        :class="menu.children?current.isCollapse==false?'':'hiddenIcon':'hiddenIcon'"
                        v-if="menu.children">
                        <template slot="title">
                            <img :src="my_getSystemImg(menu)" v-if="menu.menu_imgurl" class="menu_icon" />
                            <span slot="title"
                                v-if="!current.isCollapse">{{$t(menu.menu_name)}}{{getNum(menu.menu_name)}}</span>
                        </template>
                        <el-menu-item-group>
                            <el-menu-item :route="info" :key="info.menu_id" :index="info.menu_id+''"
                                v-for="info in menu.children">
